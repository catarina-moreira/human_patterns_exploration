...
class KnowledgeGraph:
    ...
    def create_mask_dict(self, masks: List[Mask], debug: bool = True) -> Dict:
        """
        Create a dictionary of mask bounding boxes and labels
        """
        ##method builds a dict of bounding boxes and labels from the masks.
        label_groups = defaultdict(list)
        ##Creates an empty mapping (label_groups) that will default to an empty list for any new key. We’ll use it to collect masks by their label.
        for mask in masks:
            if hasattr(mask, 'x_min') and hasattr(mask, 'labels') and mask.labels:
                raw   = mask.most_freq_label.lower().strip()
                ##Take the mask’s most_freq_label, convert to lowercase, Remove leading/trailing spaces.
                label = singularize(raw)
                label_groups[label].append(mask)
        ##Check if it has an x_min attribute, labels attribute and mask.labels is not empty.
        self.bbox_info = {}
        ##Sum up each mask’s confidence (most_freq_prob) and divide by n.
        for label, grouped_masks in label_groups.items():
            n = len(grouped_masks)
            avg_prob = sum(mask.most_freq_prob for mask in grouped_masks) / n
            avg_x_min = sum(mask.x_min for mask in grouped_masks) / n
            avg_x_max = sum(mask.x_max for mask in grouped_masks) / n
            avg_y_min = sum(mask.y_min for mask in grouped_masks) / n
            avg_y_max = sum(mask.y_max for mask in grouped_masks) / n
            ##For each of the four coordinates (x_min, x_max, y_min, y_max), compute the mean across all masks in the group.
            self.bbox_info[label] = {
                'label': label,
                'label_prob': round(avg_prob, 3),
                'bbox': {
                    'x_min': int(avg_x_min),
                    'x_max': int(avg_x_max),
                    'y_min': int(avg_y_min),
