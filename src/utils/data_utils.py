...
def normalize_fixation_duration(df):
    ...
    # Normalization: scale durations to 10..100 range:
    df['FixationDurationNorm'] = (100 -10) * (durations - minval) / (maxval - minval) + 1

    return df

def filter_data(data, imgID, partID, condition = None, group = None):
		# Filter by Image first
		filtered_data = data[data['ItemNum'] == imgID]
		...
		if 'Group' in filtered_data.columns:
			filtered_data = filtered_data[filtered_data['Group'] == group]

		# Filter by ParticipantID unless "ALL" is specified
		if "ALL" not in str(partID):
			filtered_data = filtered_data[filtered_data['ParticipantID'] == partID]
...
def process_participant_data(data, imgID, partID, condition = None, group = None, seconds = 1, rows_per_second = 3, participant_threshold = 3):
    ...
    print("Participant data before center bias removal: ", str(len(filtered_data))) 
